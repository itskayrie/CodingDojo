...
const moretone=pokémon.filter(p=>p.types.length>1)
//console.log(moretone);

const dispname=pokémon.map(p=>p.name)
//console.log(dispname);
//.reduce((count, value) => count + value, 0); // Calculate the total count .map(p => 1)
...
const beforeflying=pokémon.filter((p)=>p.types[1]==="flying").map((p)=>p.types[0]);
//console.log(beforeflying);
